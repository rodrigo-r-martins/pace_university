...
public class EmailTest2 {
    public static void main(String[] args) throws Exception {
        try {
            ...
            Map<String, TreeSet<String>> emailStore = new TreeMap<>();       // Map Object to store data
            ...
            while (scan.hasNextLine()) {
                ...
                TreeSet<String> ts;                             // TreeSet to get names in order

                if (emailStore.containsKey(key)) {
                    ts = emailStore.get(key);                   // Checking if company has already been added
                    ts.add(value);                              // Adding attendee's names
                } else {
                    ts = new TreeSet<>();                       // Adding company if it hasn't been added
                    ts.add(value);                              // Adding attendee's names
                }
                emailStore.put(key, ts);                        // Storing key and value to the Map
            }
            ...
            while (it.hasNext()) {
                ...
                TreeSet<String> t = pair.getValue();                    // Getting values of Map

                for (String s : t) {            // Checking and printing each value separated
